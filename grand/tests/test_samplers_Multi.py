...
class TestNonequilibriumGCMCSphereSamplerMultiState(unittest.TestCase):
    def setUp(self):
        comm = MPI.COMM_WORLD
        rank = comm.Get_rank()
        size = comm.Get_size()
        multi_dir = [Path(f"{utils.get_data_file('tests/methane')}/{i}") for i in range(4)]
        self.run_dir = multi_dir[rank]
        # load system
        ...
        with open(self.run_dir / "ghost_list0.dat") as f:
            line = f.readlines()[0]
        ...
        output_name_dict = {"gcmc_log": str(self.run_dir / "md.log"),
                            "dcd": str(self.run_dir / "md.dcd"),
                            "rst": str(self.run_dir / "md.rst7"),
                            "ghosts": str(self.run_dir / "md.dat"),
                            "energy": str(self.run_dir / "md.csv"),
                            }
        self.gcncmc_mover = samplers.NonequilibriumGCMCSphereSamplerMultiState(
            system=self.system,
            ...
            # number of propagation steps per perturbation step (constant Hamiltonian, relaxation)
        ...
        self.pos_answer = {0: np.array([1.5446407794952393, 1.6207016706466675, 1.3611570596694946]),
        ...
        self.vel_answer = {0: np.array([.7428833246231079, -.43068626523017883, -.17129312455654144]),
    ...
    def test_exchange_identical_U(self):
        ...
        gcncmc_mover =self.gcncmc_mover
        run_dir = self.run_dir
        g_list_answer = self.g_list_answer
        pos_answer = self.pos_answer
        vel_answer = self.vel_answer

        with open(run_dir / "state.xml", 'r') as f:
            s_restart = openmm.XmlSerializer.deserialize(f.read())
        self.topology.setPeriodicBoxVectors(s_restart.getPeriodicBoxVectors())
        ...
        state = gcncmc_mover.context.getState(getPositions=True, getEnergy=True)
        ...
        check_water_paramters(gcncmc_mover, self.topology, g_list_answer[rank])
        ...
        check_water_paramters(gcncmc_mover, self.topology, g_list)
...
def test_NPT_RE_Sampler():
    ...
    if rank > 3:
        ...
        return None
    multi_dir = [Path(f"{utils.get_data_file('tests/methane_NPT')}/{i}") for i in range(4)]
